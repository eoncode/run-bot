...
public enum UpdateChecker {
    ...
    ///
    /// Uses a dedicated ephemeral `URLSession` rather than `URLSession.shared`.
    /// `URLSession.shared` has no timeout configured — on a stalled connection
    /// (mobile hotspot, corporate proxy, flaky Wi-Fi) it would hang indefinitely.
    ///
    /// Timeouts used here:
    ///   - `timeoutIntervalForRequest`: 30 s — max time to receive the first byte.
    ///   - `timeoutIntervalForResource`: 60 s — max total time for the full response.
    ///     60 s is appropriate for a small JSON payload (<10 KB); the zip download
    ///     in `downloadUpdate` uses 300 s because it fetches a ~10 MB binary.
    private static func latestMatchingRelease(betaChannel: Bool) async -> Release? {
        ...
        sessionConfig.timeoutIntervalForResource = 60  // JSON payload only — not a zip download
