...
public enum AutoUpdater {
    ...
    ///
    /// A stale-zip sweep runs before writing the new destination, removing any
    /// prior `RunBot-*.zip` files left over from previous update cycles.
    /// Each file is < 10 MB so the sweep is effectively free; it prevents
    /// unbounded accumulation in `~/Library/Caches/io.github.runbot-hq/`
    /// without relying on macOS cache-eviction pressure.
    private static func cachedZipDestination(version: String) throws -> URL {
        ...
        // ── Stale zip sweep ──────────────────────────────────────────────────
        // Remove any RunBot-*.zip files from prior update cycles before writing
        // the new destination. At most one prior file exists at any time (one
        // update per cycle), so this is O(1) in practice and effectively free.
        //
        // This runs *before* the new destination path is computed, so the file
        // being written by the current cycle is never present yet and cannot be
        ...
        existing
            .filter { $0.lastPathComponent.hasPrefix("RunBot-") && $0.pathExtension == "zip" }
