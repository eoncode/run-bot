...
extension AutoUpdater {
    ...
    /// returns without terminating — the user is left with the running version.
    ///
    /// ## ⚠️ SPEC DEVIATION — `NSApp.terminate(nil)` vs `exit(0)` — NEEDS RESOLUTION ⚠️
    ///
    /// Issue #1794's "Decisions — not doing" table explicitly chose `exit(0)` via
    /// an external shell helper as the shutdown primitive:
    ///
    ///   > **`NSApp.terminate(nil)` instead of `exit(0)`** — `exit(0)` is the
    ///   > documented self-update pattern (Sparkle uses it)... Not adopting.
    ///
    /// The spec rationale: `NSApp.terminate(nil)` routes through
    /// `applicationShouldTerminate` / `applicationWillTerminate`, which may
    /// behave unexpectedly in a self-update handoff context. `exit(0)` was
    /// chosen precisely to bypass that machinery.
    ///
    /// This implementation inverts that decision — it uses `NSApp.terminate(nil)`
    /// and does the bundle swap in-process via `FileManager.replaceItem`, with
    /// inline comments defending `NSApp.terminate` as deliberate. **One of the
    /// two must be explicitly updated with a written rationale for the change.**
    /// Either:
    ///   a) Update issue #1794's decisions table to reflect why `NSApp.terminate`
    ///      is now preferred and why the original `exit(0)` concern no longer
    ///      applies, OR
    ///   b) Revert to the `exit(0)` + external helper pattern specified in #1794.
    ///
    /// Do NOT resolve this by silently removing this comment. The discrepancy
    /// must be documented in the issue so the design record stays accurate.
    ///
