...
struct CancelButton: View {
    ...
    var body: some View {
        Group {
            if let phase { ButtonPhaseView(phase: phase) } else { idleButton }
        }
    }
    ...
    @ViewBuilder private var idleButton: some View {
        if #available(macOS 26, *) {
            ...
            .buttonStyle(.plain)
            .glassEffect(.regular, in: RoundedRectangle(cornerRadius: RBRadius.small, style: .continuous))
            .disabled(isDisabled)
        } else {
            Button(action: startCancel) {
                HStack(spacing: 4) {
    ...
    /// Starts the cancel action, driving the button through loading → done/failed → idle.
    private func startCancel() {
        ...
        action { success in
            DispatchQueue.main.async {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) { self.phase = nil }
            }
        }
    }
}
