...
struct AddRunnerSheet: View {
    ...
    private func pickExistingFolder() {
        ...
        if let window = NSApp.keyWindow ?? NSApp.mainWindow {
            ...
            // No key or main window — fall back to runModal() so the picker still opens.
            // NOTE: runModal() blocks @MainActor for the duration of the modal loop.
            // This is acceptable for NSOpenPanel (AppKit convention), but keep the
            // fallback path narrow — prefer the sheet path above in all normal cases.
            log("AddRunnerSheet \u{203a} pickExistingFolder: no key/main window — falling back to runModal()")
    ...
    private func register() async {
        ...
        await Task.detached(priority: .userInitiated) {
            ...
            if !FileManager.default.fileExists(atPath: configPath) {
                await setStep("Downloading runner package\u{2026}")
                ...
                await setStep("Unpacking runner package\u{2026}")
            ...
            await setStep("Fetching registration token\u{2026}")
            ...
            await setStep("Configuring runner\u{2026}")
            ...
            await setStep("Registering service\u{2026}")
