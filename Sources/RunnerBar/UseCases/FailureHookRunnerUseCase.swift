...
struct FailureHookRunnerUseCase: Sendable {
    ...
    /// All token key→value pairs are collected upfront. The command string is then
    /// walked once, replacing the longest matching token at each position. This prevents
    /// a resolved value from accidentally matching a subsequent token literal (e.g. a
    /// branch named `$LOCAL_PATH` would otherwise be double-substituted in a sequential
    /// `replacingOccurrences` chain).
    internal static func resolveTokens(
        ...
        // Build token table upfront so no resolved value is ever scanned again.
        // Tokens that embed single-quoted shell values use singleQuoteEscape(); URL
        // tokens are percent-encoded and contain no shell-special characters.
        let tokens: [(key: String, value: String)] = [
            ("$LOCAL_PATH",    singleQuoteEscape(localRepoPath)),
            ("$SCOPE",         singleQuoteEscape(scope)),
            ("$BRANCH",        singleQuoteEscape(branch)),
            ("$COMMIT_SHA",    singleQuoteEscape(sha)),
            ("$RUN_ID",        singleQuoteEscape(failedRunID)),
            ("$WORKFLOW_NAME", singleQuoteEscape(workflowName)),
            ("$RUN_LINK",      runLink),
            ("$COMMIT_LINK",   commitLink),
            ("$BRANCH_LINK",   branchLink),
            ("$REPO_LINK",     repoLink),
            ("$FAILURE_LOG",   escapedLog),
        ]
        // Single-pass walk: at each index try every token key (longest-first within
        // the same prefix to be safe), emit the replacement, then advance past it.
        var result = ""
        result.reserveCapacity(command.utf8.count)
        var idx = command.startIndex
        while idx < command.endIndex {
            // Try to match a token at the current position.
            if let (_, value) = tokens.first(where: { command[idx...].hasPrefix($0.key) }) {
                result += value
                idx = command.index(idx, offsetBy: tokens.first(where: { command[idx...].hasPrefix($0.key) })!.key.count)
            } else {
