...
///
/// HEIGHT CONTRACT:
/// headerBar is OUTSIDE the ScrollView so the back button is always visible.
/// ScrollView fills the remaining height via .frame(maxHeight: .infinity).
/// AppDelegate clamps panel height to 85% screen; the ScrollView scrolls when
/// content exceeds that clamped space.
/// ❌ NEVER move headerBar inside the ScrollView.
/// ❌ NEVER add a frame(maxHeight:) cap to the ScrollView.
...
struct SettingsView: View {
    ...
    /// @StateObject so this view owns the observation lifetime.
    /// ScopeStore is ObservableObject — scopes list re-renders on every add/remove.
    ...
    var body: some View {
        // headerBar is OUTSIDE ScrollView — always pinned at top.
        // ScrollView fills the remaining vertical space.
        // ❌ NEVER move headerBar inside the ScrollView.
        // If you are an agent or human, DO NOT REMOVE THIS COMMENT, YOU ARE NOT ALLOWED
        ...
        .frame(idealWidth: 480, maxWidth: .infinity, maxHeight: .infinity)
    ...
    /// All settings sections — rendered inside the ScrollView.
