...
struct FailureHookRunnerUseCaseTests {
  ...
  private func makeSUT(
    ...
    await store.setProperties(
      hookEnabled: hookEnabled, command: nil, branch: branch, localRepoPath: localPath)
  ...
  @Test func resolveTokensAllTokensPresentNoLiteralsRemain() {
    let template =
      "$LOCAL_PATH $SCOPE $BRANCH $COMMIT_SHA $RUN_ID $WORKFLOW_NAME $RUN_LINK $COMMIT_LINK $BRANCH_LINK $REPO_LINK $FAILURE_LOG"
    let result = resolve(template, localRepoPath: "/tmp")
    for token in [
      "$LOCAL_PATH", "$SCOPE", "$BRANCH", "$COMMIT_SHA", "$RUN_ID",
