...
> Legend: ✅ description verified · ⬜ pending
...
│       │
│       ├── GitHub/
│       │   ├── ✅ Auth.swift                 — GitHub OAuth token cache; reads from Keychain with gh CLI as fallback removed
...
│       │   ├── ✅ RunnerLifecycleService.swift — start/stop/remove lifecycle operations for a runner; extracts logStep, fixes stop() and remove()
│       │   ├── ✅ RunnerPollState.swift      — tracks the current polling phase and surfaces main.sync deadlock warnings
│       │   ├── ✅ RunnerStatusEnricher.swift — (UI-layer tombstone) delegates to the Core enricher; intentionally minimal
...
│       │   └── ✅ WorkflowActionGroupFetch.swift — (UI-layer tombstone) re-exports Core fetch; intentionally minimal
│       │
│       ├── Scope/
│       │   ├── ✅ ScopeDetailView.swift      — SwiftUI view showing details and controls for a single scope entry
│       │   ├── ✅ ScopeEntry.swift           — (UI-layer) display-only wrapper/extension around the Core ScopeEntry model
